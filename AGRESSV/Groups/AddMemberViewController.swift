...
class CircularImageCellAdd: UITableViewCell {
...
class AddMemberViewController: UIViewController, UITableViewDataSource, UITableViewDelegate, UISearchBarDelegate {
    ...
    override func viewDidLoad() {
        ...
        tableView.register(CircularImageCellAdd.self, forCellReuseIdentifier: "CircularImageCellAdd")
...
//
    ...
    func tableView(_ tableView: UITableView, didSelectRowAt indexPath: IndexPath) {
        ...
        addMemberToGroup(selectedUsername: selectedUsername)
    ...
    func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell {
        guard let cell = tableView.dequeueReusableCell(withIdentifier: "CircularImageCellAdd", for: indexPath) as? CircularImageCellAdd else {
    ...
    func setCornerRadiusForVisibleCells() {
        if let visibleIndexPaths = tableView.indexPathsForVisibleRows {
            for indexPath in visibleIndexPaths {
                if let cell = tableView.cellForRow(at: indexPath) as? CircularImageCellAdd {
    ...
    func addMemberToGroup(selectedUsername: String) {
        let db = Firestore.firestore()
        ...
        userQuery.getDocuments { (userSnapshot, error) in
            ...
            groupQuery.getDocuments { (groupSnapshot, error) in
                ...
                if let groupDocument = groupSnapshot?.documents.first {
                    // Update the Group_Members array with the user's email
                    groupDocument.reference.updateData([
                        "Group_Members": FieldValue.arrayUnion([userEmail])
                    ]) { error in
                        if let error = error {
                            print("Error adding member: \(error.localizedDescription)")
                        } else {
                            print("\(userEmail) successfully added to group.")
                            ...
                            badgeCountRef.setData([:]) { error in
                                ...
                                groupBadgeCountRef.setData(["BadgeCount": 0]) { error in
                                    if let error = error {
                                        ...
                                        print("Badge count successfully set for group \(SharedGroupNameData.shared.groupName_shared!).")
                                        
                                        // Show a popup message
                                        let alertMessage = "User \(selectedUsername) has been added to the group."
