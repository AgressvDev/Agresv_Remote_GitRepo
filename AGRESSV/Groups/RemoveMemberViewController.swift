...
class CircularImageCellRemove: UITableViewCell {
...
class RemoveMemberViewController: UIViewController, UITableViewDataSource, UITableViewDelegate, UISearchBarDelegate {
    
    
    
    var groupNameBack: String = SharedGroupNameData.shared.groupName_shared
    ...
    override func viewDidLoad() {
        ...
        tableView.register(CircularImageCellRemove.self, forCellReuseIdentifier: "CircularImageCellRemove")
    ...
    func GetEmail(usernameselection: String) {
        ...
        query.getDocuments { (querySnapshot, error) in
            if error != nil {
                ...
                for document in querySnapshot!.documents {
                    ...
                    //Change for each VC
                    SharedDataEmails.sharedemails.PartnerEmail = TheEmail!
            ...
            for document in documents {
                ...
                if let username = document["Username"] as? String,
                   ...
                   let doublesrank = document["Doubles_Rank"] as? Double {
                    ...
                    if email != currentUserEmail && SharedGroupNameData.shared.group_members_array_shared.contains(email) {
                        ...
                        if email == currentUserEmail {
                            ...
                            print("Skipping user: \(username), Email: \(email) (in group members)") // Debugging
                        }
                    }
                } else {
...
//
//         
//        removeMemberFromGroup(selectedUsername: selectedUsername)
//        
//        let groupDetailVC = GroupDetailViewController()
//        // Pass data to GroupDetailViewController if needed
//        groupDetailVC.groupName = self.groupNameBack
...
//        }
    ...
    func tableView(_ tableView: UITableView, didSelectRowAt indexPath: IndexPath) {
        ...
        // Call the removeMemberFromGroup function
        removeMemberFromGroup(selectedUsername: selectedUsername) { error in
            if let error = error {
                // Handle the error (e.g., show an alert)
                print("Failed to remove member: \(error.localizedDescription)")
                // Optionally show an alert to the user
                DispatchQueue.main.async {
                    let alert = UIAlertController(title: "Error", message: "Failed to remove user: \(error.localizedDescription)", preferredStyle: .alert)
                    alert.addAction(UIAlertAction(title: "OK", style: .default, handler: nil))
                    self.present(alert, animated: true, completion: nil)
                }
            } else {
                // Successfully removed the member, navigate to GroupDetailViewController
                print("\(selectedUsername) has been successfully removed from the group.")
    ...
    func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell {
        guard let cell = tableView.dequeueReusableCell(withIdentifier: "CircularImageCellRemove", for: indexPath) as? CircularImageCellRemove else {
    ...
    func setCornerRadiusForVisibleCells() {
        if let visibleIndexPaths = tableView.indexPathsForVisibleRows {
            for indexPath in visibleIndexPaths {
                if let cell = tableView.cellForRow(at: indexPath) as? CircularImageCellRemove {
    ...
    func removeMemberFromGroup(selectedUsername: String, completion: @escaping (Error?) -> Void) {
        let db = Firestore.firestore()
        
        // Query to get the email associated with the selectedUsername
        let userQuery = db.collection("Agressv_Users").whereField("Username", isEqualTo: selectedUsername)
        
        userQuery.getDocuments { (userSnapshot, error) in
            if let error = error {
            ...
            guard let userDocument = userSnapshot?.documents.first,
                  let userEmail = userDocument.get("Email") as? String else {
                print("No user found with username: \(selectedUsername)")
                completion(nil) // No error, but no user found
                return
            }
            
            // Now that we have the email, query the Agressv_Groups collection
            let groupQuery = db.collection("Agressv_Groups").whereField("Group_Name", isEqualTo: SharedGroupNameData.shared.groupName_shared!)
            
            groupQuery.getDocuments { (groupSnapshot, error) in
                if let error = error {
                    print("Error querying group document: \(error.localizedDescription)")
                    completion(error)
                    return
                }
                
                // Check if the group document exists
                if let groupDocument = groupSnapshot?.documents.first {
                    // Update the Group_Members array by removing the user's email
                    groupDocument.reference.updateData([
                        "Group_Members": FieldValue.arrayRemove([userEmail])
                    ]) { error in
                        if let error = error {
                            print("Error removing member: \(error.localizedDescription)")
                            completion(error)
                        } else {
                            print("\(userEmail) successfully removed from group.")
                            // Define the main collection and document
                            let badgeCountRef = db.collection("Agressv_BadgeCounts").document(userEmail)

                            // First, remove the subcollection document
                            let groupBadgeCountRef = badgeCountRef.collection("GroupName_ForBadge").document(SharedGroupNameData.shared.groupName_shared!)

                            groupBadgeCountRef.delete { error in
                                if let error = error {
                                    print("Error deleting subdocument: \(error)")
                                } else {
                                    print("Subdocument for group \(SharedGroupNameData.shared.groupName_shared!) successfully deleted.")

                                    // Now remove the main document
                                    badgeCountRef.delete { error in
                                        if let error = error {
                                            print("Error deleting document: \(error)")
                                        } else {
                                            print("Document for user \(userEmail) successfully deleted.")
                                        }
                                    }
                                }
                            }
                            // Show a popup message
                            let alertMessage = "User \(selectedUsername) has been removed from the group."
                            self.showPopupMessage(alertMessage)
                    ...
                    print("No document found with Group_Name: \(SharedGroupNameData.shared.groupName_shared!)")
                    completion(nil) // No error, but no group found
                }
            }
        }
    }
    ...
    func showPopupMessage(_ message: String) {
        let alert = UIAlertController(title: "Group Update", message: message, preferredStyle: .alert)
        
        // Create the OK action
        alert.addAction(UIAlertAction(title: "OK", style: .default, handler: { _ in
            // This is where you handle the OK action
