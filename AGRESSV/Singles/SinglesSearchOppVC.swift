...
class CircularImageCell5: UITableViewCell {
...
class SinglesSearchOppVC: UIViewController, UITableViewDataSource, UITableViewDelegate, UISearchBarDelegate {
...
//        var mergedArray: [(username: String, imageData: String)] = []
    ...
    var mergedArray: [(username: String, imageData: String, username_plus_skill: String)] = []
        ...
        override func viewDidLoad() {
            ...
            // Register the custom cell class
                tableView.register(CircularImageCell5.self, forCellReuseIdentifier: "CircularImageCell5")
    ...
    private func setupLabel() {
        ...
        PlayerSearch_label.text = "Select Your Opponent"
            ...
            query.getDocuments { (querySnapshot, error) in
                if error != nil {
                    ...
                    for document in querySnapshot!.documents {
                        ...
                        //Change for each VC
                        SharedDataEmails.sharedemails.OppOneEmail = TheEmail!
...
//
    ...
    func tableView(_ tableView: UITableView, didSelectRowAt indexPath: IndexPath) {
        ...
        SharedData.shared.OppOneSelection = selectedUsername
        // Call GetEmail with a completion handler
            GetEmail(usernameselection: selectedUsername) { email in
                ...
                DispatchQueue.main.async {
                    if let SinglesGameAddVC = self.storyboard?.instantiateViewController(withIdentifier: "SinglesAddGameID") as? SinglesAddGameViewController {
                        // Pass the email to the new view controller if needed
                        SinglesGameAddVC.selectedCellValueOppOneEmail = email! // Assuming AddGameViewController has an email property
                        // Push to the AddGameViewController
                        self.navigationController?.pushViewController(SinglesGameAddVC, animated: true)
                    }
                }
            }
        }
    ...
    func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell {
        guard let cell = tableView.dequeueReusableCell(withIdentifier: "CircularImageCell5", for: indexPath) as? CircularImageCell5 else {
    ...
    func setCornerRadiusForVisibleCells() {
        if let visibleIndexPaths = tableView.indexPathsForVisibleRows {
            for indexPath in visibleIndexPaths {
                if let cell = tableView.cellForRow(at: indexPath) as? CircularImageCell5 {
